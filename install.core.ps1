...
function Start-TpServices {
    ...
    if (Test-Path $imagesTar) {
        ...
        try { docker load -i $imagesTar 2>&1 | Out-Null } catch { }
    ...
    if (-not (Test-Path $imagesTar) -or $LASTEXITCODE -ne 0) {
        ...
        try { docker compose pull --quiet 2>&1 | Out-Null } catch { }
    }

    Write-Info "Cleaning up any existing services..."
    try { docker compose down --remove-orphans 2>&1 | Out-Null } catch { }

    Write-Info "Starting services, waiting for health checks..."
    $startResult = docker compose up -d --wait --timeout 180 2>&1
    $containersUp = docker compose ps -q 2>$null | Where-Object { $_.Trim() -ne "" }
    if (-not $containersUp) {
